...
public class DictUtils
{
    ...
    public static List<SysDictData> getDictCache(String key)
    {
        ...
        if (cache instanceof List<?> listCache)
        {
            if (listCache.isEmpty() || listCache.get(0) instanceof SysDictData)
            {
                @SuppressWarnings("unchecked")
                List<SysDictData> typedList = (List<SysDictData>) listCache;
                return typedList;
            }
            return JSON.parseArray(JSON.toJSONString(listCache), SysDictData.class);
        }
        if (cache instanceof String textCache)
        {
            return JSON.parseArray(textCache, SysDictData.class);
        }
